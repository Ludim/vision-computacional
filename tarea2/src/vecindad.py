...
i = 0
with open("said.txt","r") as archivo: # este archivo se genera cuando se umbraliza la imagen
...
for w in range(anchoImag):
	...
	#for h in range(height):
		...
		if not((w,h) in visitados):
			...
			#agregar linea para el archivo que guarde este nuevo color
			while siguiente:
						...
						if not(x == i and y == j):
							if ((i >= 0 and j >= 0) and
								...
								if (not(posPix in vecindad) and not(posPix in visitados)):
									...
									# agregar linea para guardar en archivo						
									vecindad.append((i,j))
									siguiente.append((i,j))
			#print vecindad
			aux.append(vecindad)
nuevaImag.save("../img/said_VECINDAD.jpg", img.format)
